...
agentCoreFunctions CpuPlugin::aCF;
	 ...
	 *****************************************************************************/
#if defined(_AIX) || defined(_LINUX) || defined(__MACH__) || defined(__APPLE__)
...
bool CpuPlugin::read_process_cpu_time(uint64* proctime, const uint32 NS_PER_HZ) {
	...
	aCF.logMessage(info, "dummyStr = " + dummyStr);
	aCF.logMessage(info, "dummyStr.npos - 1 = " + dummyStr.npos - 1);
	aCF.logMessage(info, "dummyStr[dummyStr.npos - 1] = " + dummyStr[dummyStr.npos - 1]);
